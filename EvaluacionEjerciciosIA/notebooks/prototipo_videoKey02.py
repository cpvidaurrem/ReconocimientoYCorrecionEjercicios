...
st.title("Prototipo Optimizado - Evaluación de Ejercicios con Videos")

# Carga YOLOv8s-pose (versión 8.3.2, optimizada para inferencia con CUDA 12.6)
model_pose = YOLO(os.path.join(project_dir, 'models/yolov8s-pose.pt'))

# Define el modelo LSTM (optimizado para inferencia con no_grad)
...
model.load_state_dict(torch.load(os.path.join(project_dir, 'models/lstm_modelV2.pth')))
model.eval()
...
if uploaded_file is not None:
    ...
    while cap.isOpened():
        ...
        annotated_frame = results[0].plot()  # Dibuja box, keypoints y skeleton

        # Extrae keypoints para el buffer
        keypoints = results[0].keypoints.xy.cpu().numpy()
        if len(keypoints) > 0:
            keypoints = keypoints[0]  # Primera persona
            frame_height, frame_width = frame.shape[:2]
            keypoints = keypoints / np.array([frame_width, frame_height])  # Normaliza
            ...
            with placeholder.container():
                ...
                if len(buffer) < seq_length:
                    st.write("Recolectando frames para predicción... (Necesita 10 frames)")
                else:
                    # Predice con no_grad para optimizar VRAM
            ...
            # Si no hay detección, muestra el frame original y mensaje
            frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            img_pil = Image.fromarray(frame_rgb)
            with placeholder.container():
                st.image(img_pil, caption=f"Frame {int(cap.get(cv2.CAP_PROP_POS_FRAMES))}/{frame_count}", use_column_width=True)
                st.error("Alerta: No se detectó persona en este frame. Saltando predicción.")

        cv2.waitKey(1)

    cap.release()
    os.remove("temp_video.mp4")  # Limpia
