...
from utils import calculate_angle  # Importa la función de src/utils.py (si se usa para ángulos adicionales)
...
model.load_state_dict(torch.load(os.path.join(project_dir, 'models/best_lstm_model.pth')))
model.eval()
...
if uploaded_file is not None:
    ...
    while cap.isOpened():
        ...
        results = model_pose(frame, verbose=False)  # verbose=False para menos logs
        annotated_frame = results[0].plot()  # Dibuja bounding box, keypoints y skeleton automáticamente

        # Extrae keypoints para el buffer (si hay detección)
        ...
        if len(keypoints) > 0:
            ...
            # Calcula ángulos (opcional, para el LSTM)
            ...
            # Muestra en Streamlit (usa annotated_frame con box, keypoints y skeleton)
